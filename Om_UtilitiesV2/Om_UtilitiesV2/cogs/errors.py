    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.BotMissingPermissions):
            await ctx.send(
                ...
                + "Please add these permissions to my role ('OmUtilities') in your server settings.",
            ...
            if "The global check functions for command " in str(error):
                try:
                    await ctx.author.send(
                        "❌ You cannot use OmUtilities's commands as you are globally banned."
                    )
                except:
                    pass
        ...
        # ERROR REPORTING SYSTEM #
            ...
            await self.client.error_channel.send("<@&880478895051243547>", embed=embed)
