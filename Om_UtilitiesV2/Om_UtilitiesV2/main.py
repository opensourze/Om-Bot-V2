"This is the file to run. It contains the commands to warn/ban users from OmUtilities and background tasks."

import os
os.chdir(r"C:\Users\ombha\Desktop\Om_UtilitiesV2\Om_UtilitiesV2\cogs")
...
# Command to message a user from OmUtilities
@client.command(hidden=True)
@commands.is_owner()
async def messageuser(ctx, id: int, *, message):
    if id in client.owner_ids:
        return await ctx.send("You can't send messages to an owner through OmUtilities.")
    try:
...
if __name__ == "__main__":
    ...
    for file in os.listdir(r"C:\Users\ombha\Desktop\Om_UtilitiesV2\Om_UtilitiesV2\cogs"):
        if file.endswith(".py"):
